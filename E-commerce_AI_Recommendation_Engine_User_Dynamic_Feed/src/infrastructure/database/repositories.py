...
class PostgresProductRepository(ProductRepository):
    ...
    async def get_personalized_feed(self, user_id: str, limit: int = 20) -> List[Dict[str, Any]]:
        # Fetch user affinities (fallback to default if new user)
        ...
        # Dynamic Dot-Product Calculation in SQL
        match_score = (
        ...
        # Query, sort, and limit entirely on the DB side
        query = select(Product, match_score).order_by(desc(match_score)).limit(limit)
        result = await self.session.execute(query)
        
        # Map back to clean domain dictionaries
        feed = []
